...
默认视觉 IP 是 `Lizhi`：一只浅奶油色与浅棕色斑块分布稳定的小型垂耳兔，有深色圆点眼、清楚的小兔鼻和三瓣兔嘴。Lizhi 不是吉祥物，不是贴纸，也不是站在角落里的装饰物，而是正在认真参与系统运转的荒诞工作者。
...
这个 skill 默认使用 Ian 的 “Lizhi 怪诞正文配图”风格：
...
这些图片是旧版风格校准样例，只用于校准线条密度、留白、颜色克制和构图反复刻提醒；不要把其中角色当作 Lizhi 形象参考。使用时应该从当前文章重新发明隐喻，不要照抄旧案例的物件和构图。
...
```text
Use $ian-xiaohei-illustrations 为这篇中文文章设计并生成 5 张 Lizhi 怪诞正文配图。
```
...
每张图写清楚：放在哪段后、主题、核心意思、结构类型、Lizhi 在做什么、建议中文标注词。
...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张 Lizhi 怪诞正文配图。
...
这套正文配图 Skill，只是我用 AI 搭建个人生产系统里的一个小工具。
